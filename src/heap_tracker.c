...
static heap_operation_record_t heap_buffer[HEAP_TRACKER_BUFFER_SIZE / sizeof(heap_operation_record_t)];
...
static void flush_buffer_to_file(void) {
    ...
    if (log_file_handle == -1) {
        ...
        if (log_file_handle == -1) {
            printf("[HEAP_TRACKER] Semihosting file open failed, trying write mode...\n");
            log_file_handle = semihosting_open("/tmp/heap_trace.bin", OPEN_MODE_WB);
        }
        
        if (log_file_handle == -1) {
            printf("[HEAP_TRACKER] Semihosting failed. Falling back to console output:\n");
            ...
            for (size_t i = 0; i < buffer_index; i++) {
                ...
                printf("RECORD:%zu,OP:%u,TIME:%llu", i, (unsigned int)rec->operation, (unsigned long long)rec->timestamp_us);
                ...
                switch (rec->operation) {
                    case HEAP_OP_INIT:
                        ...
                        break;
                    case HEAP_OP_MALLOC:
                        printf(",SIZE:%" PRIu32 ",PTR:0x%" PRIx32, rec->arg1, rec->arg2);
                        break;
                    case HEAP_OP_FREE:
                        printf(",PTR:0x%" PRIx32, rec->arg1);
                        break;
                    case HEAP_OP_REALLOC:
                        printf(",OLD_PTR:0x%" PRIx32 ",SIZE:%" PRIu32 ",NEW_PTR:0x%" PRIx32, 
            ...
            printf("[HEAP_TRACKER] Semihosting file opened successfully (handle: %d)\n", log_file_handle);
        }
    }
    
    // Write buffer contents using semihosting
    int bytes_written = semihosting_write(log_file_handle, heap_buffer, 
                                         buffer_index * sizeof(heap_operation_record_t));
    size_t expected_bytes = buffer_index * sizeof(heap_operation_record_t);
    
    printf("[HEAP_TRACKER] Write details: %zu records, %zu bytes each, %zu total bytes\n",
    ...
    if (bytes_written != (int)expected_bytes) {
        ...
        printf("[HEAP_TRACKER] Successfully flushed %zu records (%zu bytes) to trace file\n", buffer_index, expected_bytes);
    }
...
void heap_tracker_init(void) {
    ...
    printf("[HEAP_TRACKER] First record - op:%u, time:%llu, args:[%u,%u,%u]\n",
           ...
           (unsigned int)init_record.arg1,
           (unsigned int)init_record.arg2, 
