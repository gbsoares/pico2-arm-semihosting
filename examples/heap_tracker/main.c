...
int main()
{
    ...
    if (test_handle != -1) {
        ...
        uint8_t test_pattern[] = {
            0x01, 0x02, 0x03, 0x04,  // Should be 0x04030201 in little-endian
                                     // uint32_t
        ...
        int written =
            semihosting_write(test_handle, test_pattern, sizeof(test_pattern));
        ...
        printf(
            "This program will write binary heap trace data to "
            "heap_trace.bin\n\n");
    } else {
        printf(
            "⚠ Raw semihosting file I/O not available - will use console "
            "output\n");
        printf(
            "This program will output heap trace data to console in structured "
            "format\n\n");
    }
    ...
    printf(
        "Check trace file for binary trace data (location depends on "
        "semihosting setup)\n");
    printf(
        "Possible locations: heap_trace.bin, /tmp/heap_trace.bin, "
        "./build/heap_trace.bin\n");
