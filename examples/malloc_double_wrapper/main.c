...
void* logged_pico_realloc(void* ptr, size_t size)
{
    ...
    if (ptr == NULL) {
        printf(
            "[REALLOC] Pico SDK: NULL -> %zu bytes (like malloc), allocated at "
            "%p\n",
            size, result);
    } else if (size == 0) {
...
int main()
{
    ...
    printf(
        "Benefits: Thread safety, heap initialization, platform "
        "optimizations\n\n");
    ...
    printf(
        "All allocations went through Pico SDK's optimized malloc "
        "implementation\n");
